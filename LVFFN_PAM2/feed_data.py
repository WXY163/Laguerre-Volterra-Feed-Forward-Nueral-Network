...
class gaussian_data:
    ...
    def xt(self,samples,memory):
        ...
        x = np.zeros((samples -memory, memory),dtype = np.float64)
    ...
    def yt(self,xt,memory,order):
        ...
        ta = np.zeros(memory)
        y = np.zeros(sample - memory,dtype=np.float64)
...
class data_load:
    ...
    def loadx(self, memory):
        xt = np.genfromtxt(self.pathx,delimiter=',', dtype=np.float64)
        if(xt.ndim >1):
            sys.exit("xdata is in wrong dimension\n")
            
        x = np.zeros((xt.size -memory, memory),dtype = np.float64)
        for i in range(xt.size - memory):
            x[i,:] = xt[i:i + memory]
        return x
    def loady(self):
        yt = np.genfromtxt(self.pathy,delimiter=',', dtype=np.float64)
        return yt.reshape(yt.size,1)
    def loadvj(self):
        vj = np.genfromtxt(self.pathvj,delimiter=',', dtype=np.float64)
        return vj
...
class tranciver_load:
    ...
    def data_read(self,memory):
        ...
        label = y[memory:]
        input_x = data[:,2]
        x = np.zeros((input_x.size - memory, memory), dtype = np.float64)
    ...
    def save_b(self, alpha, J, M, path):
        bj = self.calculate_b(alpha, J, M)
        np.savetxt(path, bj[:,::-1])
